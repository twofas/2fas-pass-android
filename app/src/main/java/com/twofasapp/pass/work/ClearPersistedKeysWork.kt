...
class ClearPersistedKeysWork(
    ...
    override suspend fun doWork(): Result {
        Flog.persist("ClearKeys", "ClearPersistedKeys: started")
        if (isStopped) {
            Flog.persist("ClearKeys", "ClearPersistedKeys: stopped before run")
            return Result.success()
        }

        vaultKeysRepository.clearPersistedVaultKeys()

        Flog.persist("ClearKeys", "ClearPersistedKeys: completed")
        return Result.success()
    }
}