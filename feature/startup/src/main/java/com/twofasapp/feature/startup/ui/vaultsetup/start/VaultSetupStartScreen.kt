...
@Composable
internal fun VaultSetupStartScreen(
    viewModel: VaultSetupStartViewModel = koinViewModel(),
    openCreateVault: () -> Unit,
) {
    LaunchedEffect(Unit) {
        viewModel.clearStartupData()
    }

    Content(
        onCtaClick = openCreateVault,
    )
}
...
internal fun Content(
    ...
    Scaffold(
        ...
        Column(
            ...
            ScreenHeader(
                title = MdtLocale.strings.setupVaultStartTitle,
                description = MdtLocale.strings.setupVaultStartDescription,
                image = painterResource(R.drawable.progress_shield_0),
            )
